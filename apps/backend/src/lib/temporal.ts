...
/**
 * Message with author info for formatting.
 */
export interface MessageForFormatting {
  authorName: string
  createdAt: Date
  content: string
}
...
/**
 * Format conversation history with timestamps and date boundaries.
 *
 * Returns messages in format:
 * — 2026-01-06 —
 * [@paul] (14:30): Hello there
 * [@jane] (14:35): Hi!
 * — 2026-01-07 —
 * [@paul] (09:00): Good morning
 */
export function formatConversationHistory(
  messages: MessageForFormatting[],
  timezone: string,
  dateFormat: DateFormat,
  timeFormat: TimeFormat
): string {
  ...
  for (const msg of messages) {
    const msgDateKey = getDateKey(msg.createdAt, timezone)

    // Insert date boundary if date changed
    if (msgDateKey !== currentDateKey) {
      const dateStr = formatDate(msg.createdAt, timezone, dateFormat)
      lines.push(`— ${dateStr} —`)
      currentDateKey = msgDateKey
    }

    // Format message with timestamp
    const time = formatTime(msg.createdAt, timezone, timeFormat)
    lines.push(`[@${msg.authorName}] (${time}): ${msg.content}`)
  }
