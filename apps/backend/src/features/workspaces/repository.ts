...
export const WorkspaceRepository = {
  ...
  async addMember(
    ...
    const result = await db.query<WorkspaceMemberRow>(sql`
      INSERT INTO workspace_members (id, workspace_id, user_id, role, slug, timezone, locale, setup_completed)
      VALUES (${params.id}, ${params.workspaceId}, ${params.userId}, ${params.role ?? "member"}, ${params.slug}, ${params.timezone ?? null}, ${params.locale ?? null}, ${params.setupCompleted ?? true})
      RETURNING id, workspace_id, user_id, role, slug, timezone, locale, setup_completed, joined_at
    `)
  ...
  async listMembers(db: Querier, workspaceId: string): Promise<WorkspaceMember[]> {
    const result = await db.query<WorkspaceMemberRow>(sql`
      SELECT id, workspace_id, user_id, role, slug, timezone, locale, setup_completed, joined_at
