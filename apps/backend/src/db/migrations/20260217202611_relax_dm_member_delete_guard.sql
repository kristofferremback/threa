-- Allow DM member row deletes for maintenance/cleanup workflows while keeping
-- identity immutable for normal updates and inserts.
...
BEGIN
  ...
  -- Allow deletes so test/data maintenance can clear rows. Runtime APIs still
  -- block DM member removal in StreamService.
  IF TG_OP = 'DELETE' THEN
    RETURN OLD;
  END IF;
