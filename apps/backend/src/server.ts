...
import { ScheduledMessagesService, createScheduledMessageWorker } from "./features/scheduled-messages"
...
export async function startServer(): Promise<ServerInstance> {
  ...
  const scheduledMessagesService = new ScheduledMessagesService({ pool, eventService, streamService })
  ...
  const scheduledMessageWorker = createScheduledMessageWorker({ scheduledMessagesService })
  jobQueue.registerHandler(JobQueues.SCHEDULED_MESSAGE_FIRE, scheduledMessageWorker, {
    tier: QueueTiers.LIGHT,
    fairness: QueueFairness.NONE,
  })
