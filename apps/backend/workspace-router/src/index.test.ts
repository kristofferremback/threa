...
describe("workspace-router", () => {
  describe("health check", () => {
    ...
    test("POST /readyz falls through to default region", async () => {
      ...
      globalThis.fetch = mock(() => Promise.resolve(new Response("proxied")))
      try {
  ...
  describe("config endpoint", () => {
    ...
    test("only responds to GET", async () => {
      ...
      try {
        ...
        const res = await worker.fetch(makeRequest("/api/workspaces/ws_123/config", "POST"), makeEnv())
        expect(globalThis.fetch).toHaveBeenCalled()
      } finally {
        globalThis.fetch = originalFetch
      }
  ...
  describe("workspace-scoped routing", () => {
    test("proxies workspace API requests to correct region", async () => {
      const originalFetch = globalThis.fetch
      const mockFetch = mock(() => Promise.resolve(new Response('{"ok":true}', { status: 200 })))
      ...
      try {
        ...
        expect(url).toBe("http://eu-north-1.backend:3002/api/workspaces/ws_123/messages")
      } finally {
        globalThis.fetch = originalFetch
      }
    })
    ...
    test("proxies to default region when workspace not in KV", async () => {
      ...
      try {
        ...
        const [url] = mockFetch.mock.calls[0] as [string, any]
        expect(url).toBe("http://localhost:3002/api/workspaces/ws_unknown/streams")
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe("avatar routing", () => {
    test("proxies avatar requests using workspace ID from path", async () => {
      const originalFetch = globalThis.fetch
      const mockFetch = mock(() => Promise.resolve(new Response("image-data")))
      globalThis.fetch = mockFetch
      try {
        const env = makeEnv({
          WORKSPACE_REGIONS: { get: mock(() => Promise.resolve("eu-north-1")) },
        })
        const res = await worker.fetch(makeRequest("/api/files/avatars/ws_123/avatar.png"), env)
        const [url] = mockFetch.mock.calls[0] as [string, any]
        expect(url).toBe("http://eu-north-1.backend:3002/api/files/avatars/ws_123/avatar.png")
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe("non-workspace routes", () => {
    test("proxies auth routes to default region", async () => {
      ...
      globalThis.fetch = mockFetch
      try {
        const res = await worker.fetch(makeRequest("/api/auth/login", "POST"), makeEnv())
        const [url] = mockFetch.mock.calls[0] as [string, any]
    ...
    test("proxies /api/workspaces (list) to default region", async () => {
      ...
      try {
        ...
        const [url] = mockFetch.mock.calls[0] as [string, any]
        expect(url).toBe("http://localhost:3002/api/workspaces")
      } finally {
        globalThis.fetch = originalFetch
      }
    })
  ...
  describe("proxy headers", () => {
    ...
    test("preserves query string through proxy", async () => {
      ...
      try {
        ...
        const [url] = mockFetch.mock.calls[0] as [string, any]
        expect(url).toBe("http://localhost:3002/api/workspaces/ws_123/messages?limit=50&before=abc")
      } finally {
        globalThis.fetch = originalFetch
      }
    })
