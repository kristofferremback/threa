...
export const workspaceApi = {
  ...
  async getBootstrap(workspaceId: string): Promise<BootstrapData> {
    const response = await fetch(`${getBaseUrl()}/api/workspace/${workspaceId}/bootstrap`, { credentials: "include" })
  ...
  async getNotifications(workspaceId: string): Promise<NotificationsResponse> {
    const response = await fetch(`${getBaseUrl()}/api/workspace/${workspaceId}/notifications`, {
      credentials: "include",
    })
  ...
  async markNotificationsRead(workspaceId: string, notificationIds: string[]): Promise<void> {
    const response = await fetch(`${getBaseUrl()}/api/workspace/${workspaceId}/notifications/read`, {
      method: "POST",
   ...
   */
  async updateProfile(workspaceId: string, data: { displayName?: string; title?: string }): Promise<void> {
    const response = await fetch(`${getBaseUrl()}/api/workspace/${workspaceId}/profile`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      credentials: "include",
  ...
  async getWorkspaceBySlug(slug: string): Promise<{ id: string; name: string; slug: string }> {
    const response = await fetch(`${getBaseUrl()}/api/workspace/by-slug/${slug}`, { credentials: "include" })
   ...
   */
  async searchUsers(workspaceId: string, query: string): Promise<Array<{ id: string; name: string; email: string }>> {
