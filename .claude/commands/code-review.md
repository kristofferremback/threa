...
- **MOST IMPORTANT: Do not check build signal or attempt to build or typecheck the app. Do not report test failures, missing test parameters, or type errors. These will run separately in CI, and CI is the gold standard. If something passes CI, it passes. Your job is to find issues CI cannot catch: logic bugs, architectural problems, CLAUDE.md violations, and data flow issues.**
